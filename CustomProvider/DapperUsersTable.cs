...
namespace core_auth.CustomProvider
{
    public class DapperUsersTable
    {
        ...
        // {
        //     await using (var cmd = new NpgsqlCommand("INSERT INTO data (some_field) VALUES (@p)", _connection))
        //     {
        ...
        public DapperUsersTable(NpgsqlConnection connection)
        {
        ...
        public async Task<IdentityResult> CreateAsync(ApplicationUser user)
        {
            ...
            int rows = await _connection.ExecuteAsync(sql,
                                                      new { user.Id, user.Email, user.EmailConfirmed, user.PasswordHash, user.UserName });
